...
map_settings_outputs = {
    # TODO: add AOVs, light groups, Cryptomatte
...
for i, layer in enumerate(layers):
    ...
    outputs = [output for setting, output in map_settings_outputs.items() if getattr(layer, setting, False)]
    for j, output in enumerate(outputs):
        node_output.file_slots.new(output)
        C.scene.node_tree.links.new(node_output.inputs[output], node_layer.outputs[output])
